...
ds_train,  info= tfds.load(ds_name,
                     ...
                     #split="train[:50%]+test[0:50%]", #omniglot
...
"""
if (ds_name=="omniglot"):
    print("limitando el dataset a 1000 categorias como maximo")
...
if (ds_name=="omniglot"):
    w2=48 #32 #48 #227 #105 #64 #32 #crop #32
    h2=48 #32 #48 #227 #105 #64 #32
...
#deconstruccion
filters= filters/reduccion
densas=densas/reduccion
...
model = tf.keras.models.Sequential([
  ...
  #adaptacion cifar (32x32)
  #layers.Conv2D(filters=int(filters/4), kernel_size=(3, 3),strides=1, activation='relu', input_shape=(h2, w2,channels2)),
...
"""
#OMNIGLOT 32x32
filters=64 #para omniglot?
model = tf.keras.models.Sequential([
  layers.Conv2D(filters=int(filters/2), kernel_size=(3, 3),activation='relu', input_shape=(h2, w2,channels2)),
  layers.MaxPooling2D((2, 2)),
  
  layers.Conv2D(filters=int(filters), kernel_size=(3, 3), padding="same",activation='relu'),
  layers.MaxPooling2D((2, 2)),
...
filters= filters/reduccion
model = tf.keras.models.Sequential([
  layers.Conv2D(filters=int(filters/2), kernel_size=(5, 5),activation='relu', input_shape=(h2, w2,channels2)),
  layers.MaxPooling2D((2, 2)),
  ...
  tf.keras.layers.Flatten(),
  tf.keras.layers.Dense(int(n*3), activation='relu'),
  #tf.keras.layers.Dropout(rate=0.5),
  #tf.keras.layers.Dense(int(n*2), activation='relu'),
  #tf.keras.layers.Dropout(rate=0.5),
  tf.keras.layers.Dense(n, activation='softmax')
...
model = tf.keras.models.Sequential([
  ...
  tf.keras.layers.Dense(int(10000), activation='relu'),
  tf.keras.layers.Dense(int(5000), activation='relu'),
  tf.keras.layers.Dense(int(3000), activation='relu'),
  tf.keras.layers.Dense(n, activation='softmax')
])
"""
...
#optimizador= tf.optimizers.SGD(lr=0.01)
print ("OPTIMIZER=", optimizador)
model.compile(optimizer=optimizador, #'adam',
              ...
              #learning rate default es 0.01. coger 0.001 es ir mas despacio
              #optimizer=tf.optimizers.SGD(lr=0.01), # lo he sacado de https://valueml.com/alexnet-implementation-in-tensorflow-using-python/
              metrics=['accuracy'])
...
print ("epocas finales:", epocas, ", split =", my_split, "batch=", batch_tamano, " samples:", samples)
print("")
